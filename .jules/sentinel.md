...
**Learning:** When attempting to fix secret scanner false positives, modifying documentation dummy keys into invalid bash syntax (e.g. `export OPENAI_API_KEY sk-...` instead of using the equals sign) breaks installation scripts. Furthermore, 'fixing' pre-existing CI errors in documentation when out-of-scope for the primary task is forbidden. If a CI workaround introduces regressions or invalid syntax, you must revert it and ignore the CI failure.
