# Jules agent shared rules — Blitztext for Linux

These are the shared rules that every agent MUST obey.

## Environment / verify

Mirror CI (`.github/workflows/ci.yml`). Run this setup block to ensure a working environment:
...
- Never touch `requirements.txt`, `pyrightconfig.json`, or CI without it being the explicit point of your task (Keeper/Polish own those).
...
- Read your `.jules/<name>.md` journal first.
- Append only *critical, codebase-specific* learnings (a real gotcha, a rejected change + why), never routine logs.
- Format:
  ```
  ## YYYY-MM-DD — [Title]
  **Learning:** …
...
Your Pull Requests MUST follow this format:
