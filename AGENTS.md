...
A roster of small, single-purpose agents that patrol this codebase on a schedule. Each agent owns **one concern**, makes **one small, reviewable change per run** (or opens an issue/report when a change isn't appropriate), and **must leave CI green**.
