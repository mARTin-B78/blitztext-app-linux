...
Mirror CI (`.github/workflows/ci.yml`) when running tests or gates locally:

```bash
sudo apt-get update && sudo apt-get install -y \
  python3-gi gir1.2-gtk-3.0 gir1.2-appindicator3-0.1 \
  libgirepository1.0-dev libgirepository-2.0-dev pkg-config \
  build-essential libcairo2-dev python3-dev
cd linux
...
- Read `.jules/<name>.md` first.
- Append only *critical, codebase-specific* learnings.
- Format:
  ## YYYY-MM-DD — [Title]
  **Learning:** ...
  **Action:** ...
