...
sudo apt-get update && sudo apt-get install -y \
  ...
  libgirepository1.0-dev pkg-config build-essential libcairo2-dev python3-dev
cd linux
...
> Tip: put that setup block in a top-level `AGENTS.md` so every Jules run starts
> from a working environment and the prompts can stay focused on the task.
