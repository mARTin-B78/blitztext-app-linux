...
def play(cfg, _notify_func):
    ...
    try:
        p1 = subprocess.Popen(curl_args, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
        try:
            subprocess.Popen(ffplay_args, stdin=p1.stdout, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        finally:
            p1.stdout.close()
