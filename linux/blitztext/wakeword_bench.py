...
def count_detections(uri: str, model: str, pcm: bytes, *, settle: float = 1.5,
    ...
    with socket.create_connection((host, port), timeout=5.0) as sock:
        ...
        while time.time() < deadline:
            ...
            buf += data
            if len(buf) > 2097152:
                raise ValueError("Buffer size exceeded 2MB without complete message")
...
def _drain_detections(buf: bytes) -> tuple[bytes, int]:
    ...
    while b"\n" in buf:
        ...
        if len(line) > 65536:
            raise ValueError("Header line exceeded 64KB")
        try:
        ...
        if plen > 1048576:
            raise ValueError("payload_length exceeded 1MB")
        if len(rest) < plen:
